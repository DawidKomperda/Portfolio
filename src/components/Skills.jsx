...
const Skills = () => {
  return (
    <section id='skills' className='md:w-10/12 md:mx-auto' >
      <h5 className='text-center text-xl xs:text-2xl text-accent '>What I learned</h5>
      <h2 className={`${styles.title} text-center text-slate-200`}> My experience</h2>
      <div className={`${styles.boxWidth} ${styles.flexCenter} ${styles.paddingY}`}>
        <div className={`${styles.flexStart} grid grid-cols-1 grid-rows-none gap-y-2 xs:gap-y-4 sm:grid-cols-2 `}>
          {expertise.map((expertise, index) => (
            <div key={expertise.title} className={`flex flex-col sm:h-full m-2 sm::m-4 ss:my-0 pb-4 items-center justify-start
              ${index === 1 ? "sm:border-r-2 border-accent sm:rounded-r-[2rem]" : ""}
              ...
              <h4 className={`text-center p-8 font-medium place-self-center text-2xl md:text-4xl text-accent sm:min-h-[160px] md:min-h-[185px] lg:min-h-[145px] xl:min-h-min `}>{expertise.title}</h4>
              <ul className={`list-none grid grid-cols-1 lg:grid-cols-2 gap-2 md:mr-12`}>
                {expertise.links.map((link) => (
                  <li key={link.Name} className={`${styles.marginX} font-normal md:text-lg
